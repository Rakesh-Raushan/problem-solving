    ...
    #this solution works but has time O(H)(O(logn) for balanced and O(n) for skewed) and space also O(H) due to call stack of recursion
        ...
        while root:
            if p.val <= root.val and root.val <= q.val:
                ...
                root = root.right
    # important thing to keep in mind is that this LCA logic of split point only works for BST
    # for general binary tree it wont work as nodes will not have the order left < node < right
